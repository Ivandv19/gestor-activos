...
exports.getHistorialActivo = async (req, res) => {
  ...
  try {
    ...
    const [activo] = await db.query('SELECT id FROM Activos WHERE id = ?', [id]);
    if (activo.length === 0) {
      return res
        .status(404)
    ...
    let whereClause = '';
    let queryParams = [id]; // Inicializamos con el ID del activo
    ...
    res.status(500).json({ error: 'Error al obtener el historial del activo.' });
  }
};
...
exports.getDatosAuxiliares = async (req, res) => {
  try {
    ...
    const [acciones] = await db.query('SELECT DISTINCT accion as nombre FROM Historial ORDER BY accion ASC');
    if (!acciones || acciones.length === 0) {
      return res.status(404).json({ error: 'No se encontraron acciones', errorCode: 'HIST_001' });
    }
    ...
    if (!usuarios || usuarios.length === 0) {
      return res.status(404).json({ error: 'No se encontraron usuarios', errorCode: 'HIST_002' });
    }
