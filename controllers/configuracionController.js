...
exports.updateConfiguracionAplicacion = async (req, res) => {
  try {
    ...
    const backupPath = path.join(__dirname, '../config/globalConfig.backup.json');
    ...
    await fs.writeFile(filePath, JSON.stringify(nuevaConfiguracion, null, 2), 'utf8');
...
exports.updatePerfilUsuario = async (req, res) => {
  try {
    ...
    const { nombre, email, departamento, contrasena_actual, nueva_contrasena, confirmar_nueva_contrasena, foto_url } = req.body;
    console.log(req.body);
    
    // Validar que la contraseña actual siempre esté presente
    if (!contrasena_actual) {
      return res.status(400).json({ error: 'La contraseña actual es obligatoria para realizar cambios.' });
    }

    // Verificar la contraseña actual
    const [usuario] = await db.query('SELECT contrasena FROM Usuarios WHERE id = ?', [userId]);
    if (usuario.length === 0) {
      return res.status(404).json({ error: 'Usuario no encontrado.' });
    }

    const contrasenaValida = await bcrypt.compare(contrasena_actual, usuario[0].contrasena);
    if (!contrasenaValida) {
    ...
    if (nueva_contrasena || confirmar_nueva_contrasena) {
      if (!nueva_contrasena || !confirmar_nueva_contrasena) {
        return res.status(400).json({ error: 'Debes proporcionar tanto la nueva contraseña como su confirmación.' });
      }
    ...
    if (updates.length === 0) {
      return res.status(400).json({ error: 'No se proporcionaron cambios válidos para actualizar.' });
    }
