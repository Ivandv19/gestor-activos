...
// Estados de garantía válidos
const estadosGarantia = ["Vigente", "Por vencer", "Vencida"];

// Schema de creación de garantía
export const createGarantiaSchema = z
	.object({
		activo_id: z.coerce
		...
		nombre_garantia: z
			...
			.min(1, { error: "El nombre de la garantía es obligatorio" }),
		fecha_inicio: z
			.string({ error: "Debe ser un texto válido" })
			.min(1, { error: "La fecha de inicio es obligatoria" }),
		fecha_fin: z
			.string({ error: "Debe ser un texto válido" })
			.min(1, { error: "La fecha de fin es obligatoria" }),
...
// Schema de actualización de garantía
export const updateGarantiaSchema = z.object({
	nombre_garantia: z
		...
		.optional(),
	estado: z
		.enum(estadosGarantia, { error: "Estado de garantía no válido" })
		.optional(),
