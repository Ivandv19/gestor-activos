...
exports.createAsignacionSchema = z.object({
	activo_id: z.coerce
	...
	fecha_asignacion: z.string({ error: "Debe ser un texto válido" }).min(1, { error: "La fecha de asignación es obligatoria" }),
	fecha_devolucion: z.string({ error: "Debe ser un texto válido" }).optional(),
});

exports.updateAsignacionSchema = z.object({
	fecha_devolucion: z.string({ error: "Debe ser un texto válido" }).optional(),
	usuario_id: z.coerce
		.number({ error: "Debe ser un número válido" })
		.int({ error: "El usuario debe ser un número válido" })
