...
// Estados y condiciones físicas válidas para un activo
...
export const createActivoSchema = z.object({
	...
	fecha_adquisicion: z
		.string({ error: "Debe ser un texto válido" })
		.min(1, { error: "La fecha de adquisición es obligatoria" }),
	...
	descripcion_garantia: z
		.string({ error: "Debe ser un texto válido" })
		.optional(),
});

// Schema de actualización de activo (todos los campos opcionales)
...
// Schema para validar etiqueta serial duplicada
export const validarEtiquetaSchema = z.object({
	etiqueta_serial: z
		.string({ error: "Debe ser un texto válido" })
		.min(1, { error: "La etiqueta serial es obligatoria" }),
});
